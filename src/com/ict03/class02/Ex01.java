...
public class Ex01 {
	public static void main(String[] args) {
		...
		String msg = "한국 ICT 인재 개발원 1 강의장";
		...
		for (int i = 0; i < msg.length(); i++) {
			...
			if (c2 >= 'a' && c2 <= 'z') {
				c2 = (char) (c2 - 32); // (char) 를 쓰는이유는 32는 정수기 때문에 c2가 int로 변해버림. 조심
			}
			System.out.print(c2);
		}
		System.out.println();
		...
		msg = "대한 I Love You 123 ";
		boolean b1 = msg.contains("Love"); // 대문자 소문자 모두구분
		if (b1) {
		...
		// ** 주의사항) 문자열이나 객체를 비교할때 '==' 를 사용하면 내용이 같냐? 가 아니라 메모리주소가 같냐? 라고 묻는거다.
		// 문자열에는 '==' 사용하면 안됨, equals 또는 equalsIgnoreCase를 사용해야한다.
		String s1 = "Korea";
		String s2 = "KOREA";
		...
		System.out.println();
		System.out.println("여기" + s1.equalsIgnoreCase(s2));
		...
		int s5 = 14;
		double s6 = 89.167;
		// String msg5 = msg4.format("%s의 시작년도는 %d년 이고 평균점수는 %.f 이다.", msg4, s5, s6);
		// System.out.println(msg5);

		// 7. getBytes() : byte[]
		// 해당 문자열을 byte[]로 변경시킨다."
		// 보통 입/출력 스트림 할 때 사용한다. (영문자 대소문자와 숫자만 가능)
		String msg6 = "java";
		...
		// 반대로 해당 byte 배열을 문자열로 만들수도 있다. (String 생성자 이용)
		String msg7 = new String(b);
		System.out.println(msg7);

		// 8. toCharArray() : char[]
		// 해당 문자열을 char[]로 변경시킨다.
		// 보통 입/출력 스트림 할 때 사용한다. (전세계 모든 문자 가능)
		...
		// 반대로 해당 char 배열을 문자열로 만들 수도 있다.
		String msg9 = new String(c);
		System.out.println(msg9);

		// 9. indexOf(int ch) : int
		// 문자를 받아서 해당문자의 위치값을 표시
		// 시작위치는 처음부터, 찾는 문자가 없으면 -이다.
		...
		// substring9int beginIndex, int endIndex) : String
		// 시작위치와 끝 위치를 받아서 해당 문자열 추출 (끝위치는 포함안됨)

		String msg18 = "010-6635-7172";
		String res4 = msg18.substring(4); // 6635-7172
		System.out.println(res4);
		String res5 = msg18.substring(4, 8); // 6635
		System.out.println(res5);
		String res6 = msg18.substring(msg18.indexOf('-') + 1, msg18.lastIndexOf('-')); // 6635
		...
		System.out.println();
/* 단순하게
		num = num.replace("7777", "XXXX");
		System.out.println(num);

		num2 = num2.replace("9999", "XXXX");
...
*/
		...
		System.out.println("============");
		// 20. trim() : String
		// 해당 문자열의 앞, 뒤 있는 공백 제거, 중간공백은 제거 못함(중간 공백은 문자취급 한다.)
		...
		// 21. String.valueOf(각종자료형) : static String
		...
		// 각종자료형에 +1;
		// System.out.println(p1+1); 오류발생! (boolean은 + 연산을 할 수 없다.)
		...
		// - 원래 의미는 기본자료형을 객체로 만들때 사용하는 클래스
		// 기본자료형 => 객체형 자동으로 변경 (오토박싱)
		// 기본자료형 => 기보낮료형 자동으로 변경 (오토언박싱)

		// 1. boolean형태의 문자열을 Boolean 형으로 변경
		// Boolean.parseBoolean("true"); Boolean.parseBoolean("나머지글자"); => false
		msg = "true";
		/*
		 * String msg = "true"; boolean 형이 아니라서 오류발생 if (msg) { break; }
		 */
		boolean a1 = Boolean.parseBoolean(msg);
		if (a1) {
			System.out.println("변경성공1111");
		} else {
			System.out.println("변경성공2222");
		}

		// 2. int형태의 문자열을 int형으로 변경 : Integer.parseInt(String s)
		msg = "100";
		int a2 = Integer.parseInt(msg);
		System.out.println(msg + 20);
		System.out.println(a2 + 2);

		// 3. double 형태의 문자열을 double형으로 변경 : Double.parseDouble(Strings)
		msg = "3.145";
		...
		System.out.println(msg + 1); // 스트링 문자값 + 1
		System.out.println(a4 + 1); // 캐릭터 숫자값-> int + 1(int)
		System.out.println((char) (a4 + 1)); // 캐릭터 + 캐릭터
		...
		// 앞자리 두자리에 19나 20
		// (gender가 1또는 2이면 1900년대생, 3 또는 4면 2000년대생
