...
public class Ex08 {
	public static void main(String[] args) {
		...
		//					key는 중복될 수 없다.(key가 중복되면 덮어쓰기 된다.)
		//					key는 호출하면 value가 나온다.
		...
		// containsKey(Obejct key) : boolean
		...
		map1.put(3, "일본"); // 키가 중복(덮어쓰기)
		System.out.println(map1);
		
		map1.put(6, "한국"); // value가 중복(상관없음)
		System.out.println(map1);
		...
		// 개선된 for 문 사용 => keySet() 사용 
		for (Integer k : map1.keySet()) {
			System.out.println(map1.get(k));
		}
		System.out.println("2");
		// iterator 를 사용 => keySet() 사용
		...
		// 2. key를 문자로 만들면 출력 시 keySet()을 사용해야 한다.
